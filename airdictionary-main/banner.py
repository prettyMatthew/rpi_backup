...
RADIO_CATEGORIES = [
    "전파 일반", "주파수 대역", "레이더", "통신", "전자전", "신호정보", "전파천문", "기타"
]

CONSTELLATION_CATEGORIES = [
    "황도 12궁", "북반구 별자리", "남반구 별자리", "계절별 별자리", "성단/성운", "관측 가이드", "기타"
]
...
CATEGORY_DISPLAY_MAP = {
    ...
    "기타": "Other",
    "황도 12궁": "Zodiac",
    "북반구 별자리": "Northern Hemisphere",
    "남반구 별자리": "Southern Hemisphere",
    "계절별 별자리": "Seasonal Constellation",
    "성단/성운": "Cluster / Nebula",
...
SECTION_FORM_CONFIG = {
    "air": {
        "headline": "Create an Air Dictionary Entry",
        "description": "This form is tailored for aircraft and strategic assets, with fields suited for platform overviews and specifications.",
        "title_placeholder": "Example: F-22 Raptor / B-2 Spirit / E-3 Sentry",
        "summary_placeholder": "Enter a short summary for the card view.",
        "description_placeholder": "Write development background, operational use, strengths, limitations, and context in Markdown.",
        "field_help": {
            "manufacturer": "Primary manufacturer or lead contractor",
            "country": "Primary operator or development nation",
            "role": "Air superiority, strike, ISR, support, etc.",
            "armament": "Typical weapons, payload, or hardpoint notes",
            "notes": "Operational notes, variants, comparisons",
        },
    },
    "radio": {
        "headline": "Create a Radio Entry",
        "description": "This form is optimized for aviation frequencies, radar systems, communications, and electronic warfare topics.",
        "title_placeholder": "예: Emergency 121.5 MHz / AN-ALQ-99 / X-band",
        "summary_placeholder": "Summarize the frequency band or EW topic.",
        "description_placeholder": "Describe usage, bandwidth, civil/military examples, and operational notes.",
        "field_help": {
            "manufacturer": "System manufacturer or standards body",
            "country": "Main user nation or international standard note",
            "role": "Communications, surveillance, navigation, jamming, SIGINT, etc.",
            "max_speed": "Leave blank if not relevant; use Notes for frequency or bandwidth details.",
            "notes": "Capture spectrum, bands, protocols, and tactical significance.",
        },
    },
    "constellation": {
        "headline": "Create a Constellation Entry",
        "description": "This form is designed for constellations, seasonal visibility, and observation notes.",
        "title_placeholder": "예: Orion / Cassiopeia / Scorpius",
        "summary_placeholder": "Summarize major stars, season, and viewing highlights.",
        "description_placeholder": "Write shape, how to find it, related mythology, and viewing period in Markdown.",
        "field_help": {
            "manufacturer": "Optional. You may use this for telescope or viewing gear suggestions.",
            "country": "Optional. Use for hemisphere or region notes.",
            "role": "Observation note, seasonal classification, or teaching context",
            "notes": "RA/Dec, observation difficulty, and related constellation notes",
        },
    },
}
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        body {
            ...
            color: var(--text);
            background:
                radial-gradient(circle at 12% 0%, rgba(56,189,248,0.22), transparent 26%),
                radial-gradient(circle at 88% 4%, rgba(124,58,237,0.18), transparent 22%),
                linear-gradient(180deg, #050b15 0%, #0a1428 16%, #edf3fb 16%, #f5f8fc 100%);
        }
        a { color: inherit; }
        ...
        .topbar {
            ...
            background: linear-gradient(180deg, rgba(6,12,24,0.88), rgba(8,17,32,0.72));
        ...
        .logo-mark {
            ...
            background: linear-gradient(135deg, var(--brand), var(--brand-2) 56%, #c084fc);
            display: grid; place-items: center; color: #fff; font-weight: 800;
            box-shadow: 0 18px 40px rgba(37,99,235,0.34);
        }
        .logo { color: #fff; text-decoration: none; font-size: 23px; font-weight: 800; letter-spacing: -0.03em; }
        .logo-sub { color: rgba(255,255,255,0.72); font-size: 12px; margin-top: 4px; }
        .nav-links { display: flex; gap: 8px; align-items: center; flex-wrap: wrap; }
        .nav-links a, .nav-links span {
            color: #fff; text-decoration: none; padding: 10px 14px; border-radius: 999px;
            font-size: 14px; border: 1px solid transparent; transition: .2s ease;
        ...
        .glass-card {
            ...
            border-radius: 30px;
            box-shadow: 0 25px 70px rgba(2,8,23,0.28);
        }
        .hero {
            padding: 36px;
            background:
        ...
        h1 { font-size: clamp(34px, 5vw, 54px); line-height: 1.02; margin-bottom: 12px; }
        ...
        .btn-link, button, .ghost-link, .pill-link {
            appearance: none; border: 0; cursor: pointer; text-decoration: none; display: inline-flex;
            align-items: center; justify-content: center; gap: 8px; transition: .2s ease;
        ...
        .btn-link:hover, button[type="submit"]:hover { transform: translateY(-1px); filter: brightness(1.03); }
        .ghost-link {
            padding: 11px 15px; border-radius: 14px;
            color: var(--text); border: 1px solid var(--line-strong);
            background: rgba(255,255,255,0.72);
        }
        .ghost-link:hover { transform: translateY(-1px); background: #fff; }
        ...
        .pill-link.active, .pill-link:hover {
            ...
            border-color: transparent; box-shadow: 0 10px 22px rgba(37,99,235,0.18);
        }
        .section-tabs { display:flex; gap:10px; flex-wrap:wrap; margin-bottom: 18px; }
        .section-tab {
            display:inline-flex; text-decoration:none; padding: 10px 14px; border-radius: 999px;
            background: rgba(255,255,255,0.82); color: var(--muted); border:1px solid var(--line);
            font-weight: 700;
        }
        .section-tab.active, .section-tab:hover {
            color:#fff; background: linear-gradient(135deg, var(--brand), var(--brand-3)); border-color: transparent;
        }
        .asset-card {
            height: 100%;
        ...
        .asset-card:hover { transform: translateY(-3px); transition: .2s ease; }
        .badge, .admin-badge, .count-chip {
            display:inline-flex; align-items:center; gap:6px;
            border-radius:999px; padding:7px 11px; font-size: 12px; font-weight: 800;
        }
        .badge { background: rgba(37,99,235,0.10); color: var(--brand); }
        .admin-badge { background: rgba(124,58,237,0.14); color: #6d28d9; }
        .count-chip { background: rgba(255,255,255,0.13); color: #fff; border:1px solid rgba(255,255,255,0.14); }
        ...
        input[type="text"], input[type="password"], input[type="file"], select, textarea {
            ...
            background: rgba(255,255,255,0.95); color: var(--text); padding: 13px 14px;
        ...
        th, td { padding: 12px 14px; border-bottom: 1px solid rgba(226,232,240,0.82); text-align:left; vertical-align: top; }
        th { background: rgba(248,250,252,0.96); font-size: 13px; text-transform: uppercase; letter-spacing: 0.04em; color: #475569; }
        .detail-image { width: 100%; max-height: 460px; object-fit: cover; border-radius: 22px; border:1px solid rgba(226,232,240,0.9); }
        ...
        .footer-banner {
            ...
            border-radius: 30px;
            background:
                linear-gradient(135deg, rgba(8,15,29,0.96), rgba(13,27,55,0.94)),
                radial-gradient(circle at top right, rgba(56,189,248,0.22), transparent 32%);
            color: #fff;
            border: 1px solid rgba(255,255,255,0.08);
            box-shadow: 0 26px 70px rgba(2,8,23,0.34), 0 0 0 1px rgba(255,255,255,0.06) inset;
        }
        .footer-banner-inner {
            display:grid;
        ...
        .logo-slot {
            ...
            text-align:center; padding: 12px; font-size: 12px; color: rgba(255,255,255,0.78);
        }
        ...
        @media (max-width: 980px) {
            ...
            .footer-banner-inner { grid-template-columns: 1fr; }
        }
        @media (max-width: 720px) {
            .main { padding-top: 24px; }
            .card, .hero { padding: 22px; border-radius: 24px; }
            .topbar-inner { padding: 12px 0; }
            .logo-column { grid-template-columns: repeat(3, minmax(0,1fr)); }
        }
    </style>
</head>
<body>
        ...
        <div class="container">
            ...
            <section class="footer-banner" id="contact-banner">
                <div style="display:flex;align-items:center;justify-content:space-between;gap:14px;flex-wrap:wrap;margin-bottom:18px;">
                    <div>
                        <div class="badge" style="background:rgba(255,255,255,0.12);color:white;">Footer Banner</div>
                        <h2 style="margin:10px 0 6px;color:white;">Contact & Service Banner</h2>
                        <p style="margin:0;color:rgba(255,255,255,0.78);">This banner is always rendered at the bottom of every page.</p>
                    </div>
                    <a href="{{ url_for('intro_page') }}" class="ghost-link" style="background:rgba(255,255,255,0.08);color:white;border-color:rgba(255,255,255,0.16);">Back to About</a>
                </div>

                <div class="footer-banner-inner">
                    <div class="logo-column">
                        {% for idx in range(3) %}
                            {% set left_files = ['army.png', 'navy.png', 'airforce.png'] %}
                            {% if logo_exists(left_files[idx]) %}
                                <div class="logo-slot" style="padding:8px;">
                                    <img src="{{ url_for('static', filename='logos/' ~ left_files[idx]) }}" alt="{{ left_files[idx] }}" style="max-width:100%;max-height:58px;object-fit:contain;">
                                </div>
                            {% else %}
                                <div class="logo-slot">
                                    <strong>US Armed Forces</strong><br>
                                    {{ left_files[idx] }}
                                </div>
                            {% endif %}
                        {% endfor %}
                    ...
                    <div class="logo-column">
                        {% for idx in range(3) %}
                            {% set right_files = ['marines.png', 'spaceforce.png', 'coastguard.png'] %}
                            {% if logo_exists(right_files[idx]) %}
                                <div class="logo-slot" style="padding:8px;">
                                    <img src="{{ url_for('static', filename='logos/' ~ right_files[idx]) }}" alt="{{ right_files[idx] }}" style="max-width:100%;max-height:58px;object-fit:contain;">
                                </div>
                            {% else %}
                                <div class="logo-slot">
                                    <strong>US Armed Forces</strong><br>
...
class SimpleAdminIndexView(AdminIndexView):
    ...
    def index(self):
        if not current_user.is_authenticated or not current_user.is_admin:
            flash("Administrator access only.", "danger")
...
def render_section_home(section):
    ...
    <section class="card hero">
        ...
        <div class="btn-row" style="margin-top:14px;">
            <a class="ghost-link" href="{{ url_for('intro_page') }}">About Page</a>
            {% if current_user.is_authenticated %}<a class="btn-link" href="{{ url_for('create_asset', section=active_section) }}">이 섹션에 Create New Entry</a>{% endif %}
        </div>
    </section>
    <section class="card">
...
def intro_page():
    ...
    <section class="glass-card card">
        <div class="hero-grid">
            <div>
                ...
                <p style="font-size:18px;line-height:1.8;color:rgba(255,255,255,0.88);margin:0;">A unified personal archive for airpower, radio spectrum and electronic warfare, and constellation observation. Search entries, browse categories, create new records, and manage the system from one site.</p>
                <div class="intro-actions">
                    <a class="btn-link" href="{{ url_for('home') }}">Open Air Dictionary</a>
                    <a class="ghost-link" style="color:white;background:rgba(255,255,255,0.1);border-color:rgba(255,255,255,0.18);" href="{{ url_for('radio_home') }}">Open Radio Section</a>
                    <a class="ghost-link" style="color:white;background:rgba(255,255,255,0.1);border-color:rgba(255,255,255,0.18);" href="{{ url_for('constellation_home') }}">Open Constellation Section</a>
                </div>
                <ul class="hero-list" style="color:rgba(255,255,255,0.82);">
                    <li>Air Dictionary: an expanded database for fighters, bombers, transports, ISR platforms, and more.</li>
                    <li>Radio: organize aviation frequencies, electronic warfare references, and radar/communications notes.</li>
                    <li>Constellation: record viewing notes, seasonal constellations, and future sky-map content.</li>
                </ul>
            </div>
            <div class="intro-panel">
                <div class="hero-kicker" style="background:rgba(255,255,255,0.12);color:white;">Archive Status</div>
                <div>
                    <span class="count-chip">Air Dictionary {{ counts['air'] }}</span>
                    <span class="count-chip">Radio {{ counts['radio'] }}</span>
                    <span class="count-chip">Constellation {{ counts['constellation'] }}</span>
                </div>
    ...
    <section class="section-card-grid">
        ...
        <article class="section-mini-card">
            ...
            <h3>Constellation</h3>
            <p class="muted">An astronomy archive for constellations and observation notes.</p>
            <a class="btn-link" href="{{ url_for('constellation_home') }}">Open</a>
        </article>
    </section>
    <section class="card">
        <div class="card soft">
        <h2>Footer Banner Preview</h2>
        <p class="muted">A permanent contact banner with six US Armed Forces logo slots is rendered at the bottom of every page. Place your image files in <code>static/logos/</code> using the exact filenames shown in the placeholders.</p>
    </div>
    ...
    '''
    return render_page(body, title="About Page", counts=counts, recent_assets=recent_assets, active_section="intro")
...
def create_asset():
    ...
    return render_page(
        body,
        title=f"Create New Entry | {section_label(requested_section)}",
    ...
    <div class="card">
        ...
        <form method="post" enctype="multipart/form-data">
            ...
            {% if asset['image_data'] %}
                ...
                <label class="checkbox-inline"><input type="checkbox" name="remove_image"> Current Image 삭제</label>
            {% endif %}
...
def delete_asset(asset_id):
    ...
    try:
        ...
        db.rollback()
        flash(f"An error occurred while deleting: {exc}", "danger")
    return redirect(url_for("admin_dashboard"))
...
def admin_dashboard():
    ...
    body = r'''
    <div class="card"><h2>Admin Dashboard</h2><p class="muted">Review user roles, publication status, and the full content inventory.</p></div>
    <div class="card">
        <h3>User List</h3>
    ...
    <div class="card">
        <h3>All Entries</h3>
        <table class="spec-table">
            <thead><tr><th>ID</th><th>섹션</th><th>Title</th><th>Category</th><th>작성자</th><th>Published</th><th>최종 수정</th><th>작업</th></tr></thead>
            <tbody>
                {% for asset in assets %}
                <tr>
                    <td>{{ asset['id'] }}</td>
    ...
    '''
    return render_page(body, title="Administrator 페이지", users=users, assets=assets, active_section="air")
...
def internal_error(error):
    ...
    <div class="card">
        ...
        <p class="muted">Administrator 작업 도중 문제가 생기면 다시 시도해보시고, DB 구조가 바뀐 경우에는 앱을 재실행해 초기화 보정이 적용되도록 해주세요.</p>
        <div class="btn-row">
    ...
    '''
    return render_page(body, title='Server Error', active_section='intro'), 500
