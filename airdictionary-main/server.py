...
SECTION_CONFIG = {
    "air": {
        ...
        "hero_title": "사관학교 러브콜 기다립니다",
        "hero_subtitle": "dodolove0429@naver.com.",
        "hero_desc": "이정도 만들었으면 솔직히 뽑아주셔야죠.",
    ...
    "radio": {
        ...
        "title": "Radio Wiki",
        "hero_title": "전파 자료실",
        "hero_subtitle": "주파수, 레이더, 통신, 전자전 정리",
        "hero_desc": "",
    ...
    "constellation": {
        ...
        "hero_title": "별자리 아카이브",
        "hero_subtitle": "별자리, 계절별 관측, 기초 천문 자료",
        "hero_desc": "",
...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .btn-link, .btn-danger, .pill-link { display: inline-block; padding: 10px 14px; text-decoration: none; color: white; border-radius: 12px; font-size: 14px; border: none; cursor: pointer; width: auto; }
        ...
        .pill-link.active { background: #2563eb; color: white; }
        .section-tabs { display: flex; flex-wrap: wrap; gap: 10px; margin-bottom: 16px; }
        .section-tab { display: inline-block; padding: 10px 14px; border-radius: 12px; text-decoration: none; background: #eef2ff; color: #1e3a8a; font-weight: 700; }
...
<body>
    <header class="topbar">
        <div class="container topbar-inner">
            <a class="logo" href="{{ url_for('home') }}">Horizon Atlas</a>
            <nav class="nav-links">
                ...
                {% if current_user.is_authenticated %}
                    <a href="{{ url_for('create_asset', section=active_section or 'air') }}">새 문서</a>
...
def render_section_home(section):
    ...
    <section class="card hero">
        ...
        <div class="btn-row" style="margin-top:14px;">
            <a class="btn-link" href="{{ url_for('create_asset', section=active_section) }}">이 섹션에 새 문서 작성</a>
        </div>
        {% endif %}
    </section>
    <section class="card">
...
def create_asset():
    ...
    <div class="card">
        ...
        <form method="post" enctype="multipart/form-data">
            ...
            <label>제목</label><input type="text" name="title" placeholder="예: B-2 Spirit / X-band / Orion" required>
            <label>분류</label>
...
def delete_asset(asset_id):
    ...
    save_history(
        ...
        asset["summary"],
        asset["description"],
        {field: (specs[field] if specs else "") for field, _ in SPEC_FIELDS},
        section_snapshot=normalize_section(asset["section"]),
    )
    db = get_db()
...
def admin_dashboard():
    ...
    <div class="card">
        <h3>전체 문서 현황</h3>
        <table class="spec-table">
            <thead><tr><th>ID</th><th>섹션</th><th>제목</th><th>분류</th><th>작성자</th><th>공개</th><th>최종 수정</th></tr></thead>
            <tbody>
                {% for asset in assets %}
                <tr>
                    <td>{{ asset['id'] }}</td>
